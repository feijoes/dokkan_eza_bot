...
class EZA:
        ...
        if not self._find_and_click(image_path, trys):
            if raise_error:
                self.device.screenshot().save(
                    f"ERROR_{datetime.datetime.now().strftime('%H_%M_%S')}.jpeg"
                )
    ...
    def End(self, trys=45):
        ...
        battle_end = self._find_dual_images(
            "./Images/End.jpeg", "./Images/FIGHT2.jpeg", trys, wait=10
        )
        if battle_end == None:
            self.device.screenshot().save(
                f"ERROR_{datetime.datetime.now().strftime('%H_%M_%S')}.jpeg"
            ...
            log_error(
                "Neither 'End' nor 'Fight2' template is present in the target image."
            )
        elif battle_end[0] == 0:
